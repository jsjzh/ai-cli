...
- **git rebase**: 变基失败时不再遗留 stash——失败后自动 `git rebase --abort` 回滚并恢复暂存的更改；恢复失败时明确提示手动 `git stash pop` 并设置非零退出码
- **git pullAndMerge**: 修复暂存恢复逻辑的两个缺陷——`stash pop` 失败不再吞掉原始错误（改为提示手动恢复并设置退出码）；恢复前先校验当前分支，不在原分支时拒绝自动 pop 避免在错误分支应用更改
